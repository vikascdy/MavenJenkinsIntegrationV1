...
import com.edifecs.epp.isc.{Address, CommandCommunicator, Isc, annotations => a}
...
object CommandAnnotationProcessor {
  ...
  def processAnnotatedCommandHandler(
    ...
    globalAllowedSources.add(CommandSource.AKKA)

    // Parse class-level annotations.
    cls.getAnnotations foreach {
      case ann: a.Akka =>
        if (!ann.enabled) globalAllowedSources.remove(CommandSource.AKKA)
      case ann: a.Rest =>
  ...
  def processAnnotatedCommandMethod(
    ...
    method.getAnnotations foreach {
      ...
      case ann: a.Command =>
        ...
        description = ann.description
      case ann: a.Akka =>
        if (ann.enabled) {allowedSources.add(CommandSource.AKKA)}
        else             {allowedSources.remove(CommandSource.AKKA)}
      case ann: a.Rest =>
