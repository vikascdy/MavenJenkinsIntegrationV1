 ...
 * <b>ldap_urls</b> is a list of space-separated LDAP URLs (<a
 * href="http://www.ietf.org/rfc/rfc2255.txt">RFC 2255</a>) that identifies the
 * LDAP server to use and the position in its directory tree where user entries
 * are located. When several LDAP URLs are specified then each is attempted, in
 * turn, until the first successful connection is established. Spaces in the
 * distinguished name component of the URL must be escaped using the standard
 * mechanism of percent character ('<code>%</code>') followed by two hexadecimal
 * digits (see {@link java.net.URI}). Query components must also be omitted from
 * the URL.
 * 
 * <p>
 * Automatic discovery of the LDAP server via DNS (<a
 * href="http://www.ietf.org/rfc/rfc2782.txt">RFC 2782</a>) is supported (once
 * DNS has been configured to support such a service). It is enabled by omitting
 * the hostname and port number components from the LDAP URL.</dd>
 * </dl>
 * </dl>
 * 
 * <p>
 ...
 * <code><b>ldap_filter</b></code> is an LDAP filter string (<a
 * href="http://www.ietf.org/rfc/rfc2254.txt">RFC 2254</a>). If it contains the
 * special token "<code><b>{USERNAME}</b></code>" then that token will be
 ...
 * authorization identity and it is assocated with the current {@link Subject}.</dd>
 * 
 ...
 * retry is made, and the failure is reported back to the calling application.</dd>
 * 
 ...
 * 
 * <p>
 * Arbitrary <a href="{@docRoot}
 * /../../../../../technotes/guides/jndi/jndi-ldap-gl.html#PROP">JNDI
...
public class CustomLdapLoginModule implements LoginModule {
    ...
    private static final ResourceBundle RESOURCE_BUNDLE = AccessController.doPrivileged(
    ...
    private static final String USERNAME_KEY = "javax.security.auth.login.";
    ...
    public void initialize(Subject subject, CallbackHandler callbackHandler,
        ...
        if (debug) {
            if (authFirst) {
                ...
                System.out.println("\t\t[LdapLoginModule] "
                        + "authentication-only mode; " + (useSSL ? "SSL enabled" : "SSL disabled"));
            } else {
    ...
    public boolean login() throws LoginException {
        ...
        if (tryFirstPass) {
            ...
            try {
                ...
                cleanState();
                if (debug) {
                    System.out.println("\t\t[LdapLoginModule] " + "tryFirstPass failed: "
                            + le.toString());
                }
            }
    ...
    public boolean commit() throws LoginException {
        ...
        if (!succeeded) {
            ...
            if (!principals.contains(adminPrincipal)) {
                principals.add(adminPrincipal);
            }
            if (debug) {
                System.out.println("\t\t[LdapLoginModule] " + "added adminPrincipal \""
                        + adminPrincipal + "\" to Subject");
            }
    ...
    private void attemptAuthentication(boolean getPasswdFromSharedState)
        ...
        try {
            ...
            adminPrincipal = new AdminPrincipal(username);

        } catch (Exception e) {
            if (debug) {
                System.out.println("\t\t[LdapLoginModule] " + "cannot create adminPrincipal: "
                        + e.getMessage());
            }
            throw (LoginException) new FailedLoginException("Cannot create adminPrincipal")
                    .initCause(e);
        }
    }
    ...
    private void getUsernamePassword(boolean getPasswdFromSharedState)
        ...
        // for jdk 1.7 use username.
        callbacks[0] = new NameCallback(RESOURCE_BUNDLE.getString("username."));
        callbacks[1] = new PasswordCallback(RESOURCE_BUNDLE.getString("password."), false);
