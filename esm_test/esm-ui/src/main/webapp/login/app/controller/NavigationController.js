Ext.define('Security.controller.NavigationController', {
    ...
    init: function() {
        ...
        this.application.on('changeurlpath', this.setNewPath, this);
    },
    ...
    onHistoryChange: function(token) {
        ...
        if (previousViewName == viewName) {
            var view =Ext.getCmp('Security-page-container').getLayout().getLayoutItems()[0];
            ...
            if (this.getPredefinedViews()[viewName]) {
                ...
                var view = Ext.create(this.getPredefinedViews()[viewName]);
                Security.setPage(view);
                this.application.setPageTitle(view.title);
                view.fireEvent('statechange', path);
            } else {
                throw token + " url fragment doesn't contain a valid view name.";
            }
        }
    },
